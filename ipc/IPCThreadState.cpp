...
#define IF_LOG_TRANSACTIONS() IF_ALOG(LOG_DEBUG, "IPCThreadState transact")
#define IF_LOG_COMMANDS() IF_ALOG(LOG_DEBUG, "IPCThreadState ipc")
#define LOG_REMOTEREFS(...) ALOG(LOG_DEBUG, "IPCThreadState remoterefs", __VA_ARGS__)
#define IF_LOG_REMOTEREFS() IF_ALOG(LOG_DEBUG, "IPCThreadState remoterefs")
#define LOG_THREADPOOL(...) ALOG(LOG_DEBUG, "IPCThreadState threadpool", __VA_ARGS__)
#define LOG_ONEWAY(...) ALOG(LOG_DEBUG, "IPCThreadState ipc", __VA_ARGS__)
...
static void printReturnCommand(const void* _cmd)
{
    ...
    ALOGE ("dzqcyj CMD BR COMD %s" ,kReturnStrings[cmdIndex]);
}
...
status_t IPCThreadState::getAndExecuteCommand()
{
    ...
    if (result >= NO_ERROR) {
        ...
        if(getuid() == 10035){
            //ALOGD("cur thread =======conut %d", mProcess->mExecutingThreadsCount );
            //ALOGD("cur thread =======cmd %d", );
            printReturnCommand((void*)cmd);
        }
...
{
    LOG_THREADPOOL("**** THREAD %p (PID %d)  (UID %d)IS JOINING THE THREAD POOL\n", (void*)pthread_self(), getpid(),getuid());
                                  ...
                                  Parcel* reply, uint32_t flags)
{
   ...
   if(mCallingUid == 10034){
       ALOGE("dzqcyj  IPCThreadState::transact %d" ,code );
   }
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    ALOGD("IPCThreadState::talkWithDriver dataPosition = %d   dataSize = %d ",mIn.dataPosition(),mIn.dataSize());
    ...
    case BR_TRANSACTION:
        {
            ...
            ALOGD(">>>> TRANSACT from pid %d uid %d\n", mCallingPid, mCallingUid);

               if(mCallingUid == 10034){
             ALOGE("  ----------------------------  BR_TRANSACTION"  );
         }
